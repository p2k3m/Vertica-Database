  ...
  "mainSteps": [
    {
      ...
      "inputs": {
        ...
        "runCommand": [
          ...
          "TOKEN=$(curl -s --connect-timeout 2 --max-time 5 -X PUT http://169.254.169.254/latest/api/token -H 'X-aws-ec2-metadata-token-ttl-seconds: 21600' || true)",
          "if [ -n \"$TOKEN\" ]; then",
          "  META=$(curl -s --connect-timeout 2 --max-time 5 -H \"X-aws-ec2-metadata-token: $TOKEN\" http://169.254.169.254/latest/dynamic/instance-identity/document || true)",
          "else",
          "  META=$(curl -s --connect-timeout 2 --max-time 5 http://169.254.169.254/latest/dynamic/instance-identity/document || true)",
          "fi",
          "echo '[debug] Instance identity document:'",
          "echo \"$META\"",
          "if [ -n \"$TOKEN\" ]; then",
          "  PUBLIC_IP=$(curl -s --connect-timeout 2 --max-time 5 -H \"X-aws-ec2-metadata-token: $TOKEN\" http://169.254.169.254/latest/meta-data/public-ipv4 || true)",
          "else",
          "  PUBLIC_IP=$(curl -s --connect-timeout 2 --max-time 5 http://169.254.169.254/latest/meta-data/public-ipv4 || true)",
          "fi",
          ...
          "timeout 60 docker ps --format 'table {{.ID}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}'",
          "timeout 60 docker inspect vertica_ce",
          "echo '[info] Recent Vertica container logs (last 100 lines)'",
          "timeout 60 docker logs --tail 100 vertica_ce || true",
          ...
          "if ! timeout 60 docker image inspect ${vertica_image} >/dev/null 2>&1; then timeout 300 docker pull ${vertica_image}; fi",
          "timeout 60 docker image ls ${vertica_image}",
          ...
          "import sys",
          "",
          "def ensure_vertica_python():",
          "    if importlib.util.find_spec('vertica_python') is not None:",
          "        print('[python] vertica_python already available', flush=True)",
          "        return",
          "",
          "    print('[python] Installing vertica_python from PyPI', flush=True)",
          "    commands = [",
          "        [",
          "            sys.executable,",
          "            '-m',",
          ...
          "            break",
          "    else:",
          "        raise SystemExit(f'Failed to install vertica_python: {last_error}')",
          "",
          "ensure_vertica_python()",
          "PY",
          ...
          "tls_mode = (os.environ.get('VERTICA_TLSMODE') or os.environ.get('DB_TLSMODE') or 'disable').strip() or 'disable'",
          ...
          "log('Testing bootstrap administrator via public IP address')",
          "with vertica_python.connect(**cfg(public_ip, bootstrap_user, bootstrap_pass)) as conn:",
          "    cur = conn.cursor()",
          "    cur.execute('SELECT CURRENT_DATABASE()')",
          "    log(f'Bootstrap user database via public IP: {cur.fetchone()[0]}')",
