...
exists_in_state(){ terraform state list | grep -q "$1"; }
...
if [ "$SG_ID" != "None" ] && [ -n "$SG_ID" ] && ! exists_in_state aws_security_group.db_sg; then
  terraform import aws_security_group.db_sg "$SG_ID" || true
fi
...
if [ -n "$ROLE_ARN" ] && ! exists_in_state aws_iam_role.ec2_role; then
  terraform import aws_iam_role.ec2_role "$ROLE_NAME" || true
fi
...
if [ -n "$PROFILE_NAME_OUT" ] && ! exists_in_state aws_iam_instance_profile.ec2_profile; then
  terraform import aws_iam_instance_profile.ec2_profile "$PROFILE_NAME" || true
fi
...
if [ "$IID" != "None" ] && [ -n "$IID" ] && ! exists_in_state aws_instance.host; then
  terraform import aws_instance.host "$IID" || true
fi
