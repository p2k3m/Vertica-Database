...
def test_admintools_license_command_variants_include_subcommands():
    ...
    assert any('license install --license' in command for command in install_variants)
    assert any('license install -l ' in command for command in install_variants)
    assert any(' -l /data/vertica/config/license.key' in command for command in install_variants)
...
def test_admintools_license_target_commands_skip_subcommands_for_upgrade():
    ...
    assert any('-l ' in command for command in commands)
...
def test_license_option_variants_include_supported_flags():
    ...
    create_variants = smoke._license_option_variants(
        '/data/vertica/config/license.key',
        include_short_flag=True,
        allow_plain_path=False,
    )
    ...
    assert '--license-path /data/vertica/config/license.key' not in variants
    assert '--license_path /data/vertica/config/license.key' not in variants
    assert '--license_file /data/vertica/config/license.key' not in variants
    assert '--license-key /data/vertica/config/license.key' not in variants
    assert '--license_key /data/vertica/config/license.key' not in variants
    assert '--license-key-file /data/vertica/config/license.key' not in variants
    assert '--license_key_file /data/vertica/config/license.key' not in variants
    assert '--license=/data/vertica/config/license.key' not in variants
    assert '--file=/data/vertica/config/license.key' not in variants
...
def test_discover_license_includes_licensing_directories(monkeypatch):
    def fake_exec(container, command, message, allow_root_fallback=True):
        assert container == 'vertica_ce'
        stdout = (
    ...
    assert '/opt/vertica/config/share/licensing/Vertica_CE.license.key' in candidates
    assert '/opt/vertica/share/licensing/license.key' in candidates
...
def test_ensure_vertica_license_installed_handles_invalid_license_status(monkeypatch):
    responses = [
        SimpleNamespace(
            returncode=0,
            stdout=(
        ...
        SimpleNamespace(
            returncode=0,
            stdout='License key /data/vertica/config/license.dat installed successfully',
            stderr='',
        ),
    ]
...
def test_attempt_creation_retries_when_license_flag_unrecognized_plural(monkeypatch):
    ...
    def fake_exec(container, command, message, allow_root_fallback=True):
        ...
        if 'export VERTICA_DB_LICENSE=' in script and not any(
            token in last_line for token in ('--license', '-l ', '-L ')
        ):
            return SimpleNamespace(returncode=0, stdout='Created', stderr='')
        return SimpleNamespace(
            returncode=1,
            stdout='',
            stderr='Command line contains unknown options /opt/vertica/config/license.key\n',
        )
    ...
    first_script = commands[0]
    final_script = commands[-1]
    final_line = final_script.splitlines()[-1]

    assert _command_contains_license_option(first_script, '/opt/vertica/config/license.key')
    assert any(
        export in final_script
        for export in (
            'export VERTICA_DB_LICENSE=',
            'export VERTICA_DB_LICENSE_FILE=',
            'export VERTICA_LICENSE=',
    ...
    assert _command_contains_license_option(final_script, '/opt/vertica/config/license.key')
    assert '--license' not in final_line
