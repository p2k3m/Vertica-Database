...
It is recommended (but not strictly required) to expose Git identity variables
such as ``GIT_AUTHOR_NAME``/``GIT_AUTHOR_EMAIL`` so the script can configure the
commit author automatically when running in ephemeral automation environments.
...
class RepositoryManager:
    ...
    def ensure_identity(self, repo_path: pathlib.Path) -> None:
        """Ensure git commits have a configured author identity."""
        from subprocess import run  # Local import
        ...
        if name:
            ...
            existing_name = read_config("user.name")
            if not existing_name:
                raise EnvironmentError(
                    "Git user.name is not configured. Set GIT_AUTHOR_NAME or GIT_COMMITTER_NAME, "
        ...
        if email:
            ...
            existing_email = read_config("user.email")
            if not existing_email:
                raise EnvironmentError(
                    "Git user.email is not configured. Set GIT_AUTHOR_EMAIL or GIT_COMMITTER_EMAIL, "
                    "or configure git globally."
                )
...
def validate_prerequisites(working_dir: pathlib.Path) -> Dict[str, str]:
    """Validate required environment variables and tooling are available."""
    ...
    except OSError as exc:  # pragma: no cover - filesystem permission errors
        raise EnvironmentError(f"Unable to create working directory {working_dir}: {exc}") from exc

    missing_optional = [var for var in OPTIONAL_GIT_IDENTITY_VARS if not os.environ.get(var)]
    if missing_optional:
        LOGGER.info(
            "Optional Git identity variables are unset (will rely on existing git config): %s",
...
def auto_fix_pipeline(owner: str, repo: str, repo_url: str, working_dir: pathlib.Path, *,
    ...
    env_values = validate_prerequisites(working_dir)
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        repo_manager.ensure_identity(repo_path)
