...
class GitHubClient:
    ...
    def latest_failed_run(self) -> Optional[WorkflowRun]:
        """Return the most recent failed run, prioritising pull requests."""
        url = f"{self.api_root}/repos/{self.owner}/{self.repo}/actions/runs"
        query_candidates = [
            {"event": "pull_request", "status": "failure", "per_page": 1},
            {"branch": "main", "status": "failure", "per_page": 1},
        ]
        for params in query_candidates:
            LOGGER.debug("Fetching workflow runs with params %s", params)
            response = self.session.get(url, params=params, timeout=30)
            response.raise_for_status()
            data = response.json()
            runs = data.get("workflow_runs", [])
            if runs:
                return self._build_workflow_run(runs[0])
        return None

    @staticmethod
    def _build_workflow_run(payload: Dict[str, object]) -> WorkflowRun:
        pull_requests = [
            pr.get("number")
            for pr in payload.get("pull_requests", [])  # type: ignore[arg-type]
            if isinstance(pr, dict) and pr.get("number") is not None
        ]
        return WorkflowRun(
            id=payload["id"],  # type: ignore[index]
            head_sha=payload["head_sha"],  # type: ignore[index]
            html_url=payload["html_url"],  # type: ignore[index]
            status=payload["status"],  # type: ignore[index]
            conclusion=payload.get("conclusion"),  # type: ignore[arg-type]
            name=payload["name"],  # type: ignore[index]
            event=str(payload.get("event", "")),
            head_branch=payload.get("head_branch"),  # type: ignore[arg-type]
            pull_requests=[int(pr) for pr in pull_requests],
        )
...
def auto_fix_pipeline(owner: str, repo: str, repo_url: str, working_dir: pathlib.Path, *,
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if run.event == "pull_request" and run.pull_requests:
            trigger = f"pull request(s) {', '.join(f'#%s' % pr for pr in run.pull_requests)}"
        elif run.head_branch:
            trigger = f"branch {run.head_branch}"
        else:
            trigger = "unknown trigger"

        LOGGER.info("Found failing run %s (%s) for %s", run.id, run.html_url, trigger)
        ...
        pr_body_lines = [
            "## Summary",
            f"- Automated fix attempt for workflow run [{run.id}]({run.html_url}).",
        ]
        if run.event == "pull_request" and run.pull_requests:
            linked = ", ".join(f"#{pr}" for pr in run.pull_requests)
            pr_body_lines.append(f"- Triggered by failing pull request run for {linked}.")
        elif run.head_branch:
            pr_body_lines.append(f"- Triggered by failing run on branch `{run.head_branch}`.")
